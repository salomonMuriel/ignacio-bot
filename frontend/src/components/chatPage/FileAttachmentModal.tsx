...
export default function FileAttachmentModal({
  ...
  const [fileTypeFilter, setFileTypeFilter] = useState<
    'all' | 'images' | 'pdfs'
  >('all');
  const [conversationFilter, setConversationFilter] = useState<'all' | string>(
    'all'
  );
  ...
  const filteredAndSortedFiles = userFiles
    .filter(file => {
      ...
      if (conversationFilter !== 'all') {
        matchesConversation =
          file.conversations?.some(
            conv => conv.conversation_id === conversationFilter
          ) || false;
      }
  ...
  const selectedFile =
    fileSelection.newFile || fileSelection.selectedExistingFile;
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      ...
                      style={{ background: 'var(--ig-surface-glass-dark)' }}
                    >
            ...
            <div className="space-y-4">
                    ...
                    {recentFiles.slice(0, 4).map(file => (
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          ...
                          {fileSelection.selectedExistingFile?.id ===
                            file.id && (
                            <div
                              className="p-1 rounded-full"
                              style={{ background: 'var(--ig-accent-primary)' }}
                            >
                              <svg
                                className="w-3 h-3"
                                style={{ color: 'white' }}
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  >
                    {searchQuery
                      ? `Search Results (${filteredAndSortedFiles.length})`
                      : `All Files (${filteredAndSortedFiles.length})`}
                  </h3>
                </div>
                ...
                {isLoadingFiles ? (
                  <div className="flex items-center justify-center py-8">
                    <div
                      ...
                      style={{ color: 'var(--ig-accent-primary)' }}
                    ></div>
                  </div>
                              ...
                              >
                                ...
                                {file.usage_count && file.usage_count > 0 && (
                                  <>
                                    <span>•</span>
                                    <span
                                      title={`Used in ${file.usage_count} conversation${file.usage_count > 1 ? 's' : ''}`}
                                    >
                                      {file.usage_count} use
                                      {file.usage_count > 1 ? 's' : ''}
                                    </span>
                                  </>
                                )}
                              </div>
            ...
            onMouseEnter={e => {
              if (canConfirm) {
                ...
                target.style.boxShadow =
                  'var(--ig-shadow-lg), var(--ig-shadow-glow)';
              }
            }}
            onMouseLeave={e => {
              if (canConfirm) {
