...
export default function PromptTemplateManager() {
  ...
  const [viewMode, setViewMode] = useState<'admin' | 'user' | 'all'>(
    user?.is_admin ? 'all' : 'user'
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingTemplate) {
        ...
        await api.promptTemplates.updatePromptTemplate(
          editingTemplate.id,
          updateData,
          user.id
        );
          ...
          <div className="flex space-x-2">
            {(['all', 'admin', 'user'] as const).map(mode => (
              ...
              >
                ...
                {mode === 'all' &&
                  ` (${adminTemplates.length + userTemplates.length})`}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="space-y-4">
        {(() => {
          ...
          if (viewMode === 'all' || viewMode === 'admin') {
            templatesToShow.push(
              ...adminTemplates.map(t => ({ ...t, isAdminTemplate: true }))
            );
          }
          if (viewMode === 'all' || viewMode === 'user') {
            templatesToShow.push(
              ...userTemplates.map(t => ({ ...t, isAdminTemplate: false }))
            );
          }
          ...
          <div className="text-center py-16">
            ...
            <p style={{ color: 'var(--ig-text-muted)' }}>
              Create your first prompt template to help users get started with
              common questions.
            </p>
          </div>
        ) : (
                      ...
                      <span
                        ...
                        style={{
                          background: template.isAdminTemplate
                            ? 'rgba(99, 102, 241, 0.2)'
                            : 'rgba(16, 185, 129, 0.2)',
                          color: template.isAdminTemplate
                            ? '#6366f1'
                            : '#10b981',
                        }}
                      >
