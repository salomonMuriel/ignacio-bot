...
async def get_prompt_templates(
    ...
    template_type: Optional[TemplateType] = Query(None, description="Filter by template type (admin/user)"),
    user_id: Optional[UUID] = Query(None, description="Filter by user (for user templates)")
) -> List[PromptTemplate]:
...
async def update_prompt_template(
    ...
    try:
        ...
        if not can_modify:
            raise HTTPException(status_code=403, detail="You don't have permission to modify this template")
