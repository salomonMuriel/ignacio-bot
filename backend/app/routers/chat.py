...
async def get_conversation(
    conversation_id: UUID, current_user: User = Depends(get_current_user)
):
            ...
            if file_content_data:
                if existing_file_record:
                    ...
                    try:
                        ...
                        print(
                            f"[CHAT] Existing file added to conversation successfully"
                        )
                    except Exception as e:
                        print(
                            f"[CHAT] ERROR: Failed to add existing file to conversation: {str(e)}"
                        )
                        raise HTTPException(
                            status_code=500,
            ...
            if hasattr(agent_result, "conversation_id"):
                if uploaded_file:
                    ...
                    try:
                        ...
                        print(
                            f"[CHAT] Existing file added to new conversation successfully"
                        )
                    except Exception as e:
                        print(
