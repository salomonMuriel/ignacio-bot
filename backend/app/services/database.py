...
from app.models.database import (
    ...
    OTPCode, OTPCodeCreate,
    UserFile, UserFileCreate,
    MessageType
)
...
class DatabaseService:
    ...
    async def get_conversation_by_id(self, conv_id: UUID) -> Optional[Conversation]:
        """Get conversation by ID"""
        response = self.client.table("conversations").select("*").eq("id", str(conv_id)).execute()
        
        if response.data:
            return Conversation(**response.data[0])
        return None
    
    async def update_conversation(self, conv_id: UUID, conv_data: ConversationUpdate) -> Optional[Conversation]:
        """Update conversation"""
        update_dict = {}
    ...
    async def get_conversation_messages(self, conv_id: UUID, limit: int = 50, offset: int = 0) -> List[Message]:
    ...
    async def create_otp(self, otp_data: OTPCodeCreate) -> OTPCode:
        ...
        response = self.client.table("otp_codes").insert({
    ...
    async def verify_otp(self, phone_number: str, code: str) -> Optional[OTPCode]:
        """Verify OTP code"""
        # Get valid, unused OTP
        response = (self.client.table("otp_codes")
                   .select("*")
        ...
        # Mark OTP as used
        self.client.table("otp_codes").update({"is_used": True}).eq("id", otp_record["id"]).execute()
    ...
    async def delete_session(self, token: str) -> bool:
        """Delete session"""
        response = self.client.table("user_sessions").delete().eq("session_token", token).execute()
        return len(response.data) > 0
    
    # File operations
    async def create_user_file(self, file_data: UserFileCreate) -> UserFile:
        """Create a new user file record"""
        response = self.client.table("user_files").insert({
    ...
    async def get_user_files(self, user_id: UUID) -> List[UserFile]:
        ...
        response = (self.client.table("user_files")
                   .select("*")
                   .eq("user_id", str(user_id))
                   .order("created_at", desc=True)
                   .execute())
    ...
    async def get_file_by_id(self, file_id: UUID) -> Optional[UserFile]:
        """Get file by ID"""
        response = self.client.table("user_files").select("*").eq("id", str(file_id)).execute()
