...
def create_project_aware_instructions(
    run_context: RunContextWrapper[ProjectContext], agent: Agent[ProjectContext]
) -> str:
...
def create_domain_specific_instructions(domain: str) -> str:
    ...
    from app.ai_system_prompts.marketing_expert_prompt import (
        create_marketing_expert_instructions,
    )
    from app.ai_system_prompts.technology_domain_expert import (
        create_technology_expert_instructions,
    )
    from app.ai_system_prompts.finance_expert_instructions import (
        create_finance_expert_instructions,
    )
    from app.ai_system_prompts.sustainability_expert_prompt import (
        create_sustainability_expert_instructions,
    )
    from app.ai_system_prompts.legal_compliance_expert_prompt import (
        create_legal_compliance_expert_instructions,
    )
    from app.ai_system_prompts.research_operations_domain_prompt import (
        create_operations_expert_instructions,
    )
    from app.ai_system_prompts.product_design_expert_prompt import (
        create_product_design_expert_instructions,
    )
    from app.ai_system_prompts.sales_expert_prompt import (
        create_sales_expert_instructions,
    )
    ...
    if domain.lower() in domain_map:
        ...
        print(
            f"[PROJECT_CONTEXT] Generated {len(instructions)} characters of instructions for {domain}"
        )
        return instructions
    else:
...
async def update_project_context(
    context: RunContextWrapper[ProjectContext], field: str, value: str
) -> str:
    ...
    try:
        if field == "project_name":
            ...
            if len(context.context.recent_activities) > 10:
                context.context.recent_activities = context.context.recent_activities[
                    -10:
                ]
        else:
...
@function_tool
async def get_project_summary(context: RunContextWrapper[ProjectContext]) -> str:
...
class ProjectContextService:
    ...
    def create_project_aware_agent(
        agent_name: str, domain: str = None, additional_tools: List = None
    ) -> Agent[ProjectContext]:
        """Create an agent with project context awareness and optional domain specialization"""
        print(
            f"[PROJECT_CONTEXT] Creating project-aware agent: {agent_name}"
            + (f" (domain: {domain})" if domain else " (general)")
        )
        ...
        if domain:
            ...
            def combined_instructions(
                ...
                context = run_context.context
                print(
                    f"[PROJECT_CONTEXT] Generating combined instructions for {agent_name} ({domain})"
                )
...
"""
                ...
                print(
                    f"[PROJECT_CONTEXT] Combined instructions: {len(combined)} total characters"
                )
                return combined

            agent = Agent[ProjectContext](
                name=agent_name, instructions=combined_instructions, tools=tools
            )
            print(
                f"[PROJECT_CONTEXT] Successfully created domain-specific agent: {agent_name}"
            )
            return agent
        else:
