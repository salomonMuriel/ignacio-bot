...
class StorageService:
    ...
    async def download_file(self, file_id: UUID, user_id: UUID) -> Optional[bytes]:
        ...
        try:
            ...
            response = self.client.storage.from_(self.bucket_name).download(file_record.file_path)
            return response
    ...
    async def get_file_url(self, file_id: UUID, user_id: UUID, expires_in: int = 3600) -> Optional[str]:
    ...
    async def delete_file(self, file_id: UUID, user_id: UUID) -> bool:
        ...
        try:
            ...
            response = self.client.storage.from_(self.bucket_name).remove([file_record.file_path])
